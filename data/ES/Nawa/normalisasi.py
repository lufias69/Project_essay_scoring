...
def ubah_simbol(teks):
    teks = str(teks)
    teks = teks.replace(".", " ").replace("}", "").replace("{", "").replace("(", "").replace(")", "").replace("-", "").replace(":", " ").replace(",", " ").replace("!", " ").replace(";", " ").replace("'", " ").replace('"', " ").replace('_', " ")
    return re.sub(' +', ' ',teks)
...
def pisahKata(kunci_jawaban, jawaban):
    ...
    for d, b in zip(d_index, b_index):
        #print(d,b)
        try:
            if jawaban_list[d-1] == " " or jawaban_list[d+1]== " ":
